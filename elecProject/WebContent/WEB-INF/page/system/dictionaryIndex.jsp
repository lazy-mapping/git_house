	...
	<HEAD>
		...
		function changetype(){
		  ...
		  /**选择类型列表的值，隐藏类型名称的文本框，作用是：此时表示的业务逻辑是在【保存】的时候，在已有的数据类型的基础上进行修改和编辑*/
		    ...
		    document.getElementById("newddlText").innerHTML=textStr;
		    /**
		    	Pub.submitActionWithForm()：传递3个参数
		    	* 参数一：表单Form2的名称
		    	* 参数二：URL连接地址
		    	* 参数三：表单Form1的名称
		    	...
		    	效果：
		    	* 在dictionIndex.jsp中存在2个表单，1个表单Form1,1个是表单Form2
		    	* 通过ajax，调用服务器，发送表单Form1中的元素作为传递给服务器的参数，在服务器进行处理，将处理后的结果，放置dictionaryEdit.jsp中
		    	* 将dictionaryEdit.jsp的所有内容，显示在dictionIndex.jsp中的Form2中

		    	感觉：异步效果，Form1不动，Form2针对Form1的选择，显示对应的数据
		    */
		    Pub.submitActionWithForm('Form2','${pageContext.request.contextPath }/system/elecSystemDDLAction_edit.do','Form1');
		  }  
	   }
